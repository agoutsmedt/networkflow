networks_to_alluv <- function(list_graph = NA,
  ...
  #' - length_cl is number of time windows a cluster exists.
  #'
  #'  #' @examples
  #'
  #' @export
  ...
  networks <- lapply(list_graph, function(tbl)(tbl %>% activate(nodes) %>% as.data.table))
  networks <- lapply(networks, function(dt)(dt[, .SD, .SDcols = c(node_key, cluster_column)]))

  alluv_dt <- rbindlist(networks, idcol = "Window")
  alluv_dt[,y_alluv:=1/.N, Window]

  if(summary_cl_stats == TRUE){
    alluv_dt[,share_cl_alluv:=.N/alluv_dt[,.N], intertemporal_name, env = list(cluster_column = cluster_column)] # share of cl in alluv
    ...
    n_years <- alluv_dt[, head(.SD, 1), .(Window, cluster_column), env = list(cluster_column = cluster_column)][,.N, cluster_column, env = list(cluster_column = cluster_column)]
    n_years <- n_years %>% rename(length_cl = N)
    alluv_dt <- merge(alluv_dt, n_years, by = c(cluster_column), all.x = TRUE) # length of cl
  }
